(function () {
    ...
    muteBtn.addEventListener(
        ...
        () => {
            ...
            muteBtn.textContent = muted
                ? window.LANG.muteMuted
                : window.LANG.muteMute;
            muteBtn.style.color = muted
                ? "#444"
                : C_DIM;
        }
    );
    ...
    function measureGrid() {
        ...
        const fs =
            r.width /
            ((targetW * cw16) / 16);
        const finalFS = Math.max(
            9,
            Math.min(Math.floor(fs), 24)
        );
        const charW =
            finalFS * (cw16 / 16);
    ...
    class Grid {
        ...
        art(a, px, py, co) {
            if (!Array.isArray(a)) {
                console.warn(
                    "Grid.art: undefined art passed from",
    ...
    function convRender() {
        ...
        const lastBoxH =
            lastBox.lines.length + 2; // top border + lines + bottom border
        ...
        const lastBoxTop =
            lastBoxBottom -
            (lastBoxH - 1); // -1 because bottom border is inclusive
        ...
        for (
            let i = 0;
            i < boxesToShow.length - 1;
            i++
        ) {
            totalPriorHeight +=
                boxesToShow[i].lines
                    .length + 2; // height of each prior box (no gap between boxes)
        }
        const topY =
            lastBoxTop -
            totalPriorHeight;
        ...
        const bxL = Util.clamp(
            bx - offset,
            0,
            W - boxW
        );
        const bxR = Util.clamp(
            bx + offset,
            0,
            W - boxW
        );
        ...
        for (const box of boxesToShow) {
            ...
            const boxEndY =
                ...
                2;
            const isVisible =
                boxEndY > 0; // At least part of the box is on-screen

            if (
                box.isChoice &&
                isVisible
            ) {
                convChoiceY1 = Math.max(
                    0,
                    cy
                );
                convChoiceBX = xPos;
            ...
            if (
                ...
                if (
                    ...
                    grid.set(
                        ...
                        "ᐯ",
                        convPlayerColor
                    ); // Canadian syllabics down triangle
                }
            } else if (
                lastBox.side === "them"
            ) {
        ...
        if (a1St === "walk") {
            ...
            if (a1ES < turns.length) {
                if (
                    ...
                    convAnchorY = psy;
                    convEncounterIndex =
                        a1EI; // Store encounter index for render adjustments
                    convPlayerColor =
                        C_PLAYER;
                    convNPCColor =
                        a1EncNPC.col ||
                    ...
                    if (
                        ...
                        if (
                            ...
                            ovTitle.style.color =
                                C_DANGER;
                            ovSub.textContent =
                                window.LANG.bannerKeepLivingLike;
                            ovHint.textContent =
                                "";
                            startBtn.textContent =
                                window.LANG.endScreenTryAgain;
                            overlay
                                .querySelectorAll(
                                    ".final"
                                )
    ...
    const A2_PA =
        window.GAME_DATA.playerArt;
    const A2_NPC_ARTS =
        window.GAME_DATA.npcArts;
    const A2_NPC_COLORS =
        window.GAME_DATA.npcColors;
    const A2_NPC =
        window.GAME_DATA.npcArts[0];
    const A2_NARC =
        window.GAME_DATA.narcArt;
    const A2_ROB =
        window.GAME_DATA.robinArt;
    ...
    function initAct2() {
        ...
        a2GenChunk(0, W * 3);
        hudLabel.textContent =
            window.LANG.hudCrewLabel;
        hudScore.textContent = "0";
        hudStatus.textContent =
            window.LANG.hudAct2Status;
        ...
        setTimeout(() => {
            if (phase === "act2") {
                ...
                bannerTimer = Math.max(
                    bannerTimer,
                    3500
                ); // reset timer after appending line
            }
        }, 1800);
    }
    ...
    function updateAct2(dt) {
        if (!a2TN)
            a2T +=
                dt; /* freeze game clock during conversations */
        updateBanner(dt);
        dialogUpdate(dt);
        if (a2TalkCD > 0)
            a2TalkCD -= dt;
        ...
        // Scroll
        a2Spd =
            0.004 + a2T * 0.00000015;
        a2WX += a2Spd * dt;
        while (a2Gen < a2WX + W + 150)
            a2GenChunk(
                a2Gen,
                a2Gen + 80
            );
        ...
        if (
            ...
            const pwx = Math.round(
                a2WX + a2PX
            );
            const newRu = Util.clamp(
                a2PRu + a2HopIntent,
                0,
                A2_NUM_LANES - 1
            );
        ...
        // Cleanup
        a2Blocks = a2Blocks.filter(
            (b) => b.wx + b.w > a2WX - W
        );
        a2Roads = a2Roads.filter(
            (r) =>
                r.wx + A2_VRW >
                a2WX - W - 10
        );
        a2NPCs = a2NPCs.filter(
            (n) => n.wx > a2WX - 20
        );
        a2Clouds = a2Clouds.filter(
            (c) => c.wx + 10 > a2WX - W
        );
        ...
        hudLabel.textContent =
            window.LANG.hudCrewLabel; /* Flash HUD on recruit */
        if (a2HudFlashT > 0) {
            hudScore.textContent =
                a2HudFlashMsg;
    ...
    function initAct2b() {
        ...
        a2bDone = false;
        a2bStoreX =
            Math.floor(45000 * 0.007) +
            W; // ~45s at base speed 0.007

        /* Generate building rows — enough for the whole level */
        a2bTopParts = a2bGenRow(
            a2bStoreX + W
        );
        a2bBotParts = a2bGenRow(
            a2bStoreX + W
        );
        a2bGenNPCs();
        dialogStack = [];
    ...
    function updateAct2b(dt) {
        ...
        for (
            ...
            a2bFloats[i].y -=
                0.002 * dt;
            if (a2bFloats[i].life <= 0)
                a2bFloats.splice(i, 1);
        }
        /* Shout timers */
    ...
    function ensureCrew() {
        ...
        if (!a2Crew) a2Crew = [];
        while (
            a2Crew.length < a2CrewCount
        ) {
            const ai = a2Crew.length;
            a2Crew.push({
                b: Math.random() * 6,
                ru: 0,
                art: window.GAME_DATA
                    .npcArts[
                    ai %
                        window.GAME_DATA
                            .npcArts
        ...
        // Build walk-in animation: robins slide in from off-screen edges
        {
            ...
            for (
                ...
                const startX =
                    side < 0
                        ? -4
                        : W + 4; // enter from left or right edge
                a3CrewOffsets.push({
                    cx: startX,
                    tx,
                    cy:
    ...
    function renderAct3() {
        ...
        const plX = a3Entering
            ? Math.round(a3PlayerX)
            : dc;
        const plY = a3Entering
            ? Math.round(
                  a3PlayerY || ly
              )
            : ly;
    ...
    function initAct4() {
        ...
        s4ExitScreenX =
            W -
            8; /* pinned to right side of screen */
        s4GrabBursts =
            []; /* per-grab starburst effects */
        ...
        S4_AISLE_TOP =
            S4_AISLE_BOT - aisleH + 1;
        S4_SHELF_BOT =
            S4_AISLE_TOP -
            1; /* bottom border of shelving unit */
        S4_SHELF_TOP = 1;
        ...
        hudLabel.textContent =
            window.LANG.hudHaulLabel;
        hudScore.textContent = "$0";
        hudScore.style.color = C_PLAYER;
        hudStatus.textContent =
            window.LANG.hudAct4Status;
    ...
    function updateAct4(dt) {
        ...
        for (
            ...
            s4RobinFloats[i].y -=
                0.002 * dt;
            if (
                s4RobinFloats[i].life <=
                0
            )
                s4RobinFloats.splice(
                    i,
                    1
                );
        }
        ...
        for (
            let i =
                s4GrabBursts.length - 1;
            i >= 0;
            i--
        ) {
            s4GrabBursts[i].t -= dt;
            if (s4GrabBursts[i].t <= 0)
                s4GrabBursts.splice(
                    i,
                    1
                );
    ...
    function initAct5() {
        ...
        for (
            ...
            if (slot === playerSlot)
                continue; // player slot, handled in render
            const targetX =
                lineStartX + slot * 3;
        ...
        a5Crew._playerX = Util.clamp(
            lineStartX + playerSlot * 3,
            2,
            W - 5
        );
        a5Crew._playerY = lineY;
        // Neighbours arrive after food is placed — centred around the fridge
        const shuffled = Util.shuffle(
            END_NAMES.slice()
        );
        a5Neighbours = [];
        const fridgeWidth =
            FRIDGE[0].length;
        const fridgeCX = Math.floor(
            W / 2
        );
        const numNeighbours = Math.min(
            4,
            shuffled.length
        );
        for (
    ...
    function initEnd() {
        ...
        const VIG_TEMPLATES =
            window.LANG.vigTemplates;

        const vig = [];
        const shuffled =
            Util.shuffle(END_NAMES);
        const templates = Util.shuffle(
            VIG_TEMPLATES.slice()
        );
        for (
            let i = 0;
            i <
            Math.min(4, a2CrewCount);
            i++
        )
            vig.push(
                templates[
        ...
        endD.lines.push({
            t,
            text: window.LANG.endCrewBrought(
                tot
            ),
            col: C_DANGER,
        });
        ...
        endD.lines.push({
            t,
            text: window.LANG.endYouCarried(
                my || 0
            ),
            col: "#bbb",
        });
    ...
    function updateCTA(dt) {
        ...
        if (
            ...
            ovTitle.style.color =
                C_PLAYER;
            ovSub.innerHTML =
                "The neighbourhood remembers.<br><br>Will you join them?";
            ovHint.textContent = "";
            startBtn.textContent =
                "PLAY AGAIN";
            overlay
    ...
    function render() {
        ...
        const floatBaseY = Math.floor(
            H * FLOAT_STYLE.yRatio
        );
        const floatMaxW = W - 6;
        const playful =
            phase === "act4" ||
    ...
    ovTitle.textContent =
        window.LANG.overlayTitle;
    ovSub.innerHTML =
        window.LANG.overlaySub;
    ovHint.innerHTML =
        window.LANG.overlayHint;
    startBtn.textContent =
        window.LANG.playBtn;
