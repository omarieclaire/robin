      (function () {
        ...
        function renderBanner() {
          ...
          const tH = lines.length + 2,
            startY = Math.max(1, Math.floor(H / 6 - tH / 2));
        ...
        function convRender() {
          ...
          //dialogue box narrower
          const boxW = Math.min(W - 12, 24);
          const innerW = boxW - 4;
            ...
            turns: [
              {
                who: "p",
                texts: [
                  "when did bread get so expensive?",
                  "ugh, when did bread get so expensive?",
                  "ostie, when did bread get so darn expensive?",
                  "crisse, when did BREAD get so expensive??",
                  "CÂLICE. when did BREAD get this expensive??",
                ],
              },
              { who: "n", text: "I use a coupon app" },
              { who: "n", text: "I'll send you the link" },
            ],
          },
          {
            turns: [
              {
                ...
                texts: [
                  "$12 for flour?",
                  "$12 for freaking flour??",
                  "ostie de... $12 for FLOUR?",
                  "crisse, TWELVE DOLLARS for flour??",
                  "CÂLICE. $12 for flour??!",
                ],
              },
              { who: "n", text: "yeah, and??" },
            ...
            turns: [
              {
                ...
                texts: [
                  ...
                  "ostie, somebody has to do something about this",
                  "CRISSE. somebody needs to do something",
                  "CÂLICE DE TABARNAK. someone has to do something",
                ],
              },
              { who: "n", text: "stealing from a thief isn't stealing" },
              { who: "p", texts: ["what?", "...what?", "ostie... what does that mean", "câlice. what are you saying", "TABARNAK. say that again"] },
            ],
          },
        ];
        ...
        function a2GenChunk(from, to) {
          ...
          for (let ri = 0; ri < A2_NUM_LANES; ri++) {
            for (let nx = from + Util.randInt(20, 35); nx < to; nx += Util.randInt(25, 45)) {
              ...
              /* Narc ambient tells */
              const ambLine = tp === "narc" ? DM.draw(DECK_AMB_NARC) : npcKind === "hungry" ? DM.draw(DECK_AMB_HUNGRY) : DM.draw(DECK_AMB_ANGRY);
        ...
        function updateAct2(dt) {
          ...
          if (a2TN) {
            ...
            if (a2TP === 0 && a2TT > 400) {
              ...
              convVisible = true;
              DM.startConv();
              convAddLine(DM.draw(DECK_GREET), "you", convPlayerColor);
              a2TP = 12;
              a2TT = 0;
            }
            ...
            // ── TP 12: NPC replies based on type ──
              ...
              const line =
                a2TN.tp === "narc" ? DM.draw(DECK_NARC_HELLO) : a2TN.kind === "angry" ? DM.draw(DECK_ANGRY_HELLO) : DM.draw(DECK_HUNGRY_HELLO);
            ...
            // ── TP 13: show first pitch choices ──
              ...
              a2ChoiceLabels = [prefix + DM.draw(DECK_ANGRY_PITCH, { boost: true }), prefix + DM.draw(DECK_HUNGRY_PITCH, { boost: true })];
            ...
            // ── TP 15: invite line after pitch, before NPC resolves ──
            else if (a2TP === 15 && a2TT > 900) {
              convAddLine(DM.draw(DECK_INVITE), "you", convPlayerColor);
              a2TP = 2;
              a2TT = 0;
            }

            // ── TP 2: resolve first pitch ──
            else if (a2TP === 2 && a2TT > 1200) {
              const matched = (a2Choice === 0 && a2TN.kind === "angry") || (a2Choice === 1 && a2TN.kind === "hungry");

              if (a2TN.tp === "narc") {
                // Narc always asks for more
                convAddLine(DM.draw(DECK_SAY_MORE), "them", convNPCColor);
                a2TP = 20;
                a2TT = 0;
              } else if (matched) {
                // Matched energy — 50% join now, 50% ask for more
                if (Math.random() < 0.5) {
                  // Immediate join — joinPhrase first or consentPhrase first
                  const jp = DM.draw(DECK_JOIN),
                  ...
                  DM.draw(DECK_SAY_MORE)
                  // convAddLine(Util.pick(D_SAY_MORE), "them", convNPCColor);
                    convAddLine(DM.draw(DECK_SAY_MORE), "them", convNPCColor);
                ...
                // Mismatch — both lines merge into one box
                console.log("connect");

                convAddLine(DM.draw(DECK_MISMATCH) + " " + DM.draw(DECK_NO_BYE), "them", convNPCColor);
                a2TP = 7;
                a2TT = 0;
              }
            }
            ...
            // ── TP 21: immediate join — jp first, now add cp ──
              ...
              convAddLine(DM.draw(DECK_CONSENT), "them", convNPCColor);
              a2TP = 8;
              a2TT = 0;
            }

            // ── TP 22: immediate join — cp first, now add jp ──
            else if (a2TP === 22 && a2TT > 1000) {
              convAddLine(DM.draw(DECK_JOIN), "them", convNPCColor);
              a2TP = 8;
              a2TT = 0;
            }

            // ── TP 20: NPC asked for more — show second choices ──
            else if (a2TP === 20 && a2TT > 1000) {
              a2ChoiceLabels = [DM.draw(DECK_STRONGER_PITCH, { boost: true }), DM.draw(DECK_BACK_OFF)];
              convShowChoices(a2ChoiceLabels);
              a2Choice = -1;
              a2TP = 23;
              a2TT = 0;
            ...
            else if (a2TP === 24 && a2TT > 1200) {
              if (a2Choice === 1) {
                // Backed off — silent exit
                DM.endConv();

                addFloat(Util.pick(["Played it safe", "Better safe than sorry?", "It could have been a narc"]), 0, 0, "#a80");
                a2TN.st = "done";
                a2TN.cd = 9999;
                a2TN = null;
                a2TalkCD = 500;
                ...
                // Stronger pitch to narc — reveal
                convAddLine(DM.draw(DECK_NARC_REV), "them", C_DANGER);
                a2TP = 9;
                a2TT = 0;
              } else {
                // Stronger pitch to normal NPC — join or defer
               if (Math.random() < 0.6) {
  const jp = DM.draw(DECK_JOIN), cp = DM.draw(DECK_CONSENT);
  if (Math.random() < 0.5) {
    convAddLine(jp, "them", convNPCColor);
    a2TP = 21;
  } else {
  ...
  a2TP = 10;
        ...
        function updateAct5(dt) {
          ...
          if (clickPending && a5P === 2) {
            ...
            for (let _i = 0; _i < 6; _i++)
              burstGood(_fx + Math.floor(Math.random() * _fw), _fy + Math.floor(Math.random() * FRIDGE.length), C_TEAL, 7);
            triggerFlashGood();
            a5T = 0;
        ...
        function initEnd() {
          ...
          endD = { tot, my, as: s4AlyScore, crew: a2CrewCount, vig, fed };
        ...
        function renderCTA() {
          const CTA_STORY = [
            ...
            { t: 2200, text: "robbed a metro grocery store in montreal", col: C_DANGER },
            ...
            { t: 10700, text: "maybe your neighbourhood is next?", col: C_ORANGE },
          ];
        ...
        ovSub.innerHTML = "walk around? talk to your neighbours? anything could happen<br><br>";
        ovHint.innerHTML =
        ...
        startBtn.addEventListener("click", async () => {
          await audio.unlock();
          await audio.preload(["click", "music"]);
          audio.play("music", { loop: false, volume: 0.3 });
          startGame();
        });
