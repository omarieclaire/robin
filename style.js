/* Box styles, palette, colors, timing values. */
  ...
  // const BANNER_BOX = { tl: "╭", tr: "╮", bl: "╰", br: "╯", h: "─", v: "│" };
  /* Double-lines = structured conversation feel */
  const CONV_BOX = {
    tl: "\u2554",
    tr: "\u2557",
    bl: "\u255A",
  ...
  /* Single-lines = quiet, ambient feel */
  const DIALOG_BOX = {
    tl: "\u256D",
    tr: "\u256E",
    bl: "\u2570",
  ...
  /* Square corners signal solid/clickable boxes */
  const SHARP_BOX = {
    tl: "\u250C",
    tr: "\u2510",
    bl: "\u2514",
  ...
  const C_DANGER = PAL.danger || "#ff5555"; // narcs, heat, bust, security
  const C_TEAL = PAL.teal || "#5cbdbd"; // crew, fridge, info, success moments
  const C_ORANGE = PAL.rally || "#00ffbb"; // rally banners, CTA accent
  const C_VIOLET = PAL.violet || "#a97bff"; // rally-the-neighbourhood interstitial only
  ...
  const C_CONV_NPC = PAL.convNpc || "#7a8aaa";

  const HAT_CHAR = "ƛ";
  const HAT_COLOR = C_ORANGE;

  // Shared Act 4/5 storefront palette.
  const STORE_SIGN_COLS = ["#f5a032", "#f2c94c", "#e8944a", "#ffb347", "#f7dc6f"];
  function storeSignColor(i) {
    return STORE_SIGN_COLS[i % STORE_SIGN_COLS.length];
  }
  // Flash partner, same palette.
  function storeSignColorFlash(i) {
    return STORE_SIGN_COLS[(i + Math.floor(STORE_SIGN_COLS.length / 2)) % STORE_SIGN_COLS.length];
  }
  const CAT_GLYPHS = ["ஹ"];

  function playerPulseColor(t) {
    // 1.4Hz glow, pass any act timer
  ...
  const T = {
    ...
    hold: 1500, // wait for heavy line to land
    ...
    npcPer: 260, // extra ms per word beyond npcMin
  ...
  const FLOAT_STYLE = {
    ...
    fadeStart: 0.15 /* fraction of life when fade starts */,
    padX: 1,
  };
