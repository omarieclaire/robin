...
const tagsOf = (item) => {
  ...
  return (item.tags || []).filter((t) => t != null && t !== ""); // strips holes from leading commas
};
...
const callSites = [
  ...
  // also filtered by helloTags at TP13
  ...
  // TP 141: filler, filtered by pitch tags
  ...
  // TP14 narc skeptical: filtered by inviteTags/badRead
  { deck: "DECK_SAY_MORE_SKEPTICAL",
    from: { kind: "union", of: ["DECK_F_INVITE","DECK_BAD_READ"] } },

  // TP14 mismatch: filtered by bad-read tags
  ...
  // also appended to mismatch line
  { deck: "DECK_NO_BYE", from: { kind: "deck", name: "DECK_BAD_READ" } },

  // TP14 matched: filtered by inviteTags
  ...
  // TP25: filtered by bail-line tags
  { deck: "DECK_BAIL_RESPONSE", from: { kind: "deck", name: "DECK_BACK_OFF_EARLY" } },

  // TP10: NPC return greeting, no filter
  { deck: "DECK_RETURN", from: { kind: "none" } },

  // ambient decks: drawAmb, no filter
...
for (const name of allDeckNames) {
  const arr = srcOf(data[name]);
  if (!arr) continue;
  // avoids double-reporting shared D_/DECK_ arrays
  if (seenBugArrays.has(arr)) continue;
  seenBugArrays.add(arr);
  // prefers the DECK_ name when reporting
...
for (const id of ids) {
  ...
  if (callSiteDecks.has(id)) continue; // used per our call-graph model
