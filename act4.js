  ...
  const A2B_NPC_COL = ["#0ff", "#f0f", "#ff0", "#0f8", "#f80", "#8f0", "#c66aff", "#f08", "#08f"];
  ...
  function a2bCalcLayout() {
    ...
    A2B_BOT_H = Math.max(5, Math.floor(H * 0.18)); // keeps buildings near true bottom
    // road starts at buildings' bottom edge
  ...
  function a2bGenNPCs() {
    ...
    if (a2bKiosks) {
      for (const k of a2bKiosks) {
        if (k.wx > a2bStoreX) continue; // post-store bookend is decorative
    ...
    for (let nx = 60; nx < a2bStoreX - 5; nx += Util.randInt(spacing * 2, spacing * 2 + 8)) {
  ...
  function initAct4() {
    ...
    a2bDone = false;
    a2bStoreX = Math.floor(96000 * 0.007) + W; // distance tuned for ~60-70s at ramp speed

    // covers full level length
    ...
    if (_roadH2b >= 10) {
      ...
      if (_segPool.length > 0) {
        const _baseY = Math.floor((A2B_ROAD_Y1 + A2B_ROAD_Y2) / 2) + 4; // ground line; buildings align to it
        const _mkSeg = (kx) => {
        ...
        for (let kx = 20; kx < a2bStoreX - 5; ) {
          const seg = _mkSeg(kx);
          if (kx + seg.w > a2bStoreX - 6) break; // would crowd the approach
          a2bKiosks.push(seg);
          kx += seg.w + 1 + Util.randInt(10, 15); // weaving gap between blocks
        }
  ...
  function updateAct4(dt) {
    ...
    if (a2bDone) {
      ...
      // centers player on the store door
    ...
    Footsteps.setRate(a2bSpd / (0.009 * A2B_MOBILE_SPD_MULT));
    const ms = 0.025;
    const tapStep = 2; // adjust to feel right
    ...
    if (clickPending && phase === "act4") {
      ...
      if (!Device.isMobile) {
        ...
        if (clickSX < a2bPX - 3) a2bPX -= 2; // screen-only; world never rewinds
        else if (clickSX > a2bPX + 3) {
          a2bWX += 2; // forward click keeps hurry boost
          a2bPX += 1;
        }
      }
    }
    // mobile left/right via hold, no mobileMoveX
      ...
      for (const n of a2bNPCs) {
        ...
        if (Math.abs(n.wx - pwx) < A2B_HIT_W && Math.abs(n.wy - a2bPY) < A2B_HIT_H) {
          if (n.narc) {
            ...
            Banner.show(window.LANG.bannerHitNarc, C_DANGER, 1800, true);
            ...
            if (a2bHt >= A2B_MH) {
              ...
              a2bSpd = 0;
              a2bBusted = true;
              setTimeout(() => triggerCorruptBust("busted", initAct4), A2B_REVEAL_PAUSE_MS);
              return;
            }
          } else {
            n.st = "joined";
    ...
    if (a2bStoreX + Math.floor(STO_W / 2) - a2bWX <= Math.round(a2bPX) && !a2bDone) {
      ...
      const _restMidY = Math.floor((A2B_ROAD_Y1 + A2B_ROAD_Y2) / 2); // targets where player will stop
  ...
  // deterministic per tile avoids flicker
  // clouds must avoid this zone
  function _a2bSkyClouds(scrollX, tile = 130, perTile = 2, exclude = null) {
  ...
  function renderAct4(opts = {}) {
    ...
    for (let x = 0; x < W; x++) {
      const wx = x + mtScrollX; // world x; float allows smooth scroll

      const period = 220;
      const phase = ((wx % period) + period) % period;
      const norm = phase / period;

      const dome = Math.exp(-Math.pow((norm - 0.3) * 3.0, 2)); // Outremont-inspired secondary shoulder
    ...
    _a2bSkyClouds(mtScrollX, 80, 3, crossZone);
    // sun drawn after clouds, uncovered
// grid.art([
//     " ⟍  │ ⟋ ",
//     "   ︵     ",
//     "⎯(^‿^)⎯",
...
// ], W - 8, 1, C_COIN);
    ...
    if (peakScreenX >= 0) {
      const crossArt = [" | ", "-+-", " | "];
      const cx = peakScreenX - 1; 
      const cy = peakScreenY - 3; 
    ...
    for (const n of a2bNPCs) {
      ...
      // flash only when close
      const _a2bNpcClose = n.narc && Math.abs(n.wx - (a2bWX + a2bPX)) < 18;
      const _a2bNpcCol = _a2bNpcClose ? (Math.floor(a2bT / 130) % 2 === 0 ? C_DANGER : "#fff") : n.col;
    ...
    if (a2bWX > a2bStoreX - W - 20) {
      ...
      if (stsx < W + 10) {
        ...
        for (let _ri = 0; _ri < STORE.length; _ri++) {
          ...
          // matches Act 5's storefront palette
          ...
          for (let _ci = 0; _ci < _row.length; _ci++) {
            ...
            else grid.set(stsx + _ci, stY + _ri, " ", "#000"); // block mountain bleed
          }
        }
      }
    }
    ...
    if (!opts.skipPlayerCrew) {
      ...
      for (let i = 0; i < a2bMob.length; i++) {
        ...
        if (m._settling) {
          ...
          m._settleT += 16; // approx frame time, fine for easing
          ...
          const baseOX = -4 - Math.floor(i / 3) * 2; // matches resting orbit's baseOX below
          ...
          // legs animate below t=0.7, then still
          _mobFrame = [...(m.art || [m.ch, "\u03C6"])];
          if (t < 0.7) {
            _mobFrame[1] = Math.floor(a2bT / 200 + m.b * 30) % 2 === 0 ? _mobFrame[1] : "\u20B3";
          }
        } else if (m.isCat) {
          // cat pads alongside, doesn't tumble
          const prowl = Math.sin(a2bT / 400 + m.b) * 1.4;
          const baseOX = -4 - Math.floor(i / 3) * 2; // matches resting orbit's baseOX
          mx = Math.round(ppx + baseOX + prowl);
          my = Math.round(ppy);
          _mobFrame = [...(m.art || [m.ch, "\u03C6"])];
        } else {
          // katamari-style tight orbit around cluster
          const clusterR = 2;
          const angle = a2bT / 600 + m.b;
          const orbitX = Math.sin(angle + i) * clusterR;
          const orbitY = Math.cos(angle + i) * (clusterR * 0.35);
          const baseOX = -4 - Math.floor(i / 3) * 2; // keeps orbit clear of player
        ...
        if (mx >= 0 && mx < W && my >= 0 && my < H) {
          ...
          grid.art(_mobFrame, mx, my, col, m.isCat); // cat sprite always faces forward
        }
      }

      // same 2-row art as Act 3
    ...
    for (const n of a2bNPCs) {
      ...
      const ty = n.wy - bh - 0;
      // matches the sprite flash
      const boxCol = n.narc ? (Math.floor(a2bT / 130) % 2 === 0 ? C_DANGER : "#fff") : n.col;
      if (ty >= A2B_ROAD_Y1 && ty + bh < A2B_ROAD_Y2) {
        ...
        grid.text(DIALOG_BOX.tl + DIALOG_BOX.h.repeat(bw - 2) + DIALOG_BOX.tr, tx, ty, boxCol);
        for (let li = 0; li < lines.length; li++) {
          grid.text(DIALOG_BOX.v + " ".repeat(bw - 2) + DIALOG_BOX.v, tx, ty + 1 + li, boxCol);
          grid.text(lines[li], tx + 1, ty + 1 + li, boxCol);
        }
        grid.text(DIALOG_BOX.bl + DIALOG_BOX.h.repeat(bw - 2) + DIALOG_BOX.br, tx, ty + 1 + lines.length, boxCol);
      }
    }
