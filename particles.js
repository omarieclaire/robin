/* Sparks, floats, screen-flash triggers for grid. */
  const SPARK_CH = ["*", "+", "·", "×", "◦"];
  ...
  function burstGood(cx, cy, color, n) {
    ...
    for (let i = 0; i < (n || 10); i++) {
      const a = Math.PI + (Math.PI * i) / ((n || 10) - 1);
  ...
  function addFloat(t, x, y, co, local, sound) {
    // act6 floats vanish faster (food)
    const life = phase === "act6" ? 550 : FLOAT_STYLE.life;
    if (!local && phase !== "act6") audio.play(sound || "paper");
    floats.push({
      text: t,
      color: co,
      life,
    ...
    for (let fi = 0; fi < floats.length; fi++) {
      ...
      if (boxed) {
        ...
        for (let li = 0; li < lines.length; li++) {
          ...
          grid.text(lines[li], bx + 1 + FLOAT_STYLE.padX + pad, by + 1 + li, _floatTextCol); // bold smears glyphs at small sizes
        }
        grid.text(fBox.bl + fBox.h.repeat(bw - 2) + fBox.br, bx, by + 1 + lines.length, f.color);
      } else {
        for (let li = 0; li < lines.length; li++) {
          const pad = Math.floor((lineW - lines[li].length) / 2);
          grid.text(lines[li], bx + FLOAT_STYLE.padX + pad, by + li, f.color); // bold smears glyphs at small sizes
        }
      }
    }
  }
  ...
  function popupRender() {
    for (const p of _inlinePopups) {
      ...
      // Matches act 2 dialog box style
