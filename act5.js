  ...
  const RA2 = ["@", "\u0126"];
  let a3T, a3CrewOffsets, a3Entering, a3PlayerX, a3PlayerY, a3CrewBannerShown, a3RallyStarted, a3Clouds;
  let a3FirstCrewStarted, a3CrewSettleEl;
  ...
  function initAct5(opts = {}) {
    ...
    a3PlayerAppearAt = 0;
    a3CrewStartAt = 0; // when crew animation begins (ms)
    a3PlayerAppearAt = 0;
    ...
    a3EntryBurst = false;
    if (a3CrewSettleEl) audio.stop(a3CrewSettleEl);
    a3FirstCrewStarted = false;
    a3CrewSettleEl = null;
    a3HatsOn = false;
    a3HatQueue = [];
    a3HatQueueT = 0;
...

    {
      ...
      const shuffled = [...window.GAME_DATA.cloudShapes].sort(() => Math.random() - 0.5);
      const picked = Device.isMobile ? shuffled.slice(0, 3) : shuffled;
      const groupCount = Device.isMobile ? 2 : 3;
      const groupW = W / groupCount;
      a3Clouds = picked.map((art, i) => {
        const group = i % groupCount;
        const cw = art[0].length;
        const center = groupW * (group + 0.5);
        const jitter = (Math.random() - 0.5) * groupW * 0.6;
        return {
          art,
          row: 1 + Math.floor(Math.random() * 4),
          x: Math.round(Util.clamp(center + jitter, 2, W - cw - 2)),
  ...
  function updateAct5(dt) {
    ...
    if (!a3PlayerAppearAt && !Banner.seq && Banner.timer <= 0) {
      ...
      a3CrewStartAt = a3T + 1000; // crew starts walking 1s after player
      audio.play("playerArrive");
    }
    ...
    if (a3CrewOffsets && !a3Entering) {
      for (const c of a3CrewOffsets) {
        if (a3CrewStartAt && a3T - a3CrewStartAt > c.delay) {
          if (!a3FirstCrewStarted) {
            a3FirstCrewStarted = true;
            a3CrewSettleEl = audio.play("crewSettle", { loop: true, volume: 0.35 });
          }
          c.cx = Util.lerp(c.cx, c.tx, 0.07);
          c.cy = Util.lerp(c.cy, c.ty, 0.07); // vertical too
      ...
      // lineup completion triggers its own banner
      if (!a3CrewBannerShown && a3CrewStartAt && a3CrewOffsets.length > 0 && a3CrewOffsets.every((c) => c.arrived)) {
        a3CrewBannerShown = true;
        if (a3CrewSettleEl) {
          audio.stop(a3CrewSettleEl);
          a3CrewSettleEl = null;
        }
        Banner.showSequence([
          { t: window.LANG.bannerCrewAssembled || "crew assembled", c: C_TEAL, d: 2000 },
          { pause: true, d: 400 },
          { t: window.LANG.bannerWeNeedDisguises || "we need disguises", c: C_TEAL, d: 2500 },
        ]);
      }
    }
    ...
    if (a3HatQueue.length > 0) {
      ...
      if (a3HatQueueT <= 0) {
        ...
        // snaps 80ms delay to render frames
    ...
    if (clickPending || input.justPressed("action")) {
      ...
      if (_hatPromptVisible && !a3HatsOn && a3HatQueue.length === 0 && a3T > _allArrivedAt + 1000) {
      ...
      } else if (a3HatsOn && a3PlayerHatted && _bannerClear2 && !a3Entering) {
        clickPending = false;
        audio.play("click");
        ...
        // discards input before prompt appears
        clickPending = false;
      }
    }
  }
  function renderAct5(opts = {}) {
    ...
    const roofH = 1;
    // roof matches mobile's narrower store box
    const roofBaseW = Device.isMobile ? STO_W - 4 : STO_W;
    const roofBaseX = Device.isMobile ? scx + 2 : scx;
    const roofOverhang = Device.isMobile ? 1 : 3;
    const roofW = roofBaseW + roofOverhang * 2;
    let roofScallop = "";
    while (roofScallop.length < roofW) roofScallop += "▛▀▜";
    roofScallop = roofScallop.slice(0, roofW);
    const roofX = roofBaseX - roofOverhang;
    const roofY = stY - roofH;

    if (roofY > 4) {
      for (const c of a3Clouds || []) grid.art(c.art, c.x, c.row, c.col);

      const sunX = scx < W - scx - STO_W ? Math.max(2, W - 7) : 2;
      grid.art([
        "   ︵     ", 
        "༺⸨^⸩༻",
        "   ︶     ",
    ...
    {
      const allBuildings = window.GAME_DATA.buildings;
      const tallestH = Math.max(...[0, 1, 2, 3].map((k) => allBuildings[(a2CrewCount + k) % allBuildings.length].art.length));
      const mtBaseY = Math.min(sY - 1, stY + STORE.length - tallestH - 1);
      const mtBoost = Device.isMobile ? 2 : 0; // fixes mobile peak clipping too low
      const mtMaxH = Math.min(mtBaseY - 5, Math.floor(H * 0.22) + mtBoost); // leaves headroom for the cross
      if (mtMaxH > 3) {
        let peakX = -1,
          peakY = 99999;
        for (let x = 0; x < W; x++) {
          const norm = x / W; // keeps the peak off the edge
          const dome = Math.exp(-Math.pow((norm - 0.28) * 3.0, 2));
          const shoulder = Math.exp(-Math.pow((norm - 0.62) * 5.0, 2)) * 0.35;
          const hillH = Math.round((dome + shoulder) * mtMaxH);
          const topY = mtBaseY - hillH;
          if (topY < peakY) {
            peakY = topY;
            peakX = x;
          }
          for (let dy = Math.max(0, topY); dy <= mtBaseY && dy < H; dy++) {
            const depth = dy - topY;
            const frac = mtMaxH > 0 ? depth / mtMaxH : 0;
            let ch, col;
            if (depth === 0) {
              ch = "௳";
              col = "#3d5230"; // ridge
        ...
        // Mount Royal cross, same as Act 4
        if (peakX >= 0 && peakY - 3 >= 0) grid.art([" | ", "-+-", " | "], peakX - 1, peakY - 3, "#f0e8c0");
      }
    }

    grid.text(roofScallop, roofX, roofY, "#9a6d4a");
    ...
    for (let ri = 0; ri < STORE.length; ri++) {
      ...
      const rowCol = storeFlash ? storeSignColorFlash(ri) : storeSignColor(ri);
      for (let ci = 0; ci < row.length; ci++) {
        const inTreeZone = Device.isMobile && (ci < _treeOffset || ci >= _treeOffset + _rawStoreW);
        if (row[ci] === " ") {
          if (!inTreeZone) grid.set(scx + ci, stY + ri, " ", "#000"); // block mountain bleed
          continue;
        }
        if (inTreeZone) {
    ...
    if (!opts.skipPlayerCrew) {
      ...
      // draws crew using saved art/color
      ...
      for (let ri = 0; ri < art.length; ri++) {
        ...
        for (let ci = 0; ci < row.length; ci++) {
          ...
          if (ch !== " ") {
            ...
            grid.set(x + ci, y + ri, " ", "#000"); // block mountain bleed
          }
        }
      }
    }

    let leftBldgEdge = scx;
    if (scx > 18) {
      const b = pickBuilding(a2CrewCount);
      const bw = b.art[0].length;
      const bx = scx - (bw + 10);

      // avoid overlapping the store
      if (bx + bw < scx) {
        drawBuilding(b, bx, "#4a3d78", 0);
        leftBldgEdge = bx;
      }
    }

    let rightBldgEdge = scx + STO_W;
    if (scx + STO_W + 18 < W) {
      const b = pickBuilding(a2CrewCount + 1);
      const bw = b.art[0].length;
      const bx = scx + STO_W + 10;

      drawBuilding(b, bx, "#5c4d92", 1); // slight depth variation
      rightBldgEdge = bx + bw;
    }

    if (!Device.isMobile) {
      if (leftBldgEdge > 12) {
        const b2 = pickBuilding(a2CrewCount + 2);
        const bw2 = b2.art[0].length;
        const bx2 = leftBldgEdge - (bw2 + 4);
        if (bx2 > 0) drawBuilding(b2, bx2, "#332a5c", 0);
      }
      if (rightBldgEdge + 12 < W) {
        const b3 = pickBuilding(a2CrewCount + 3);
        const bw3 = b3.art[0].length;
        const bx3 = rightBldgEdge + 4;
        if (bx3 + bw3 < W) drawBuilding(b3, bx3, "#453a70", 1);
      }
    }
  ...
  function _transitionAct5ToAct6() {
    ...
    runActBoundary({
      ...
      intro: (done) => {
        ...
        }, { spawnPool: _pool, peppersMs: 700, excludeXY: to, overlay: _walkCellsOverlay(staged, to), simmer: true });
      },
    });
  }
