  ...
  let a1PX, a1PY, a1CX, a1CY, a1T, a1St, a1NPCs, a1AmbNPCs;
  let a1Pokes = [];
  let a1EncNPC, a1EI, a1ES, a1ST2;
  let a1Path, a1PI, a1PA, a1NP, a1FullPath;
  let a1IdleTimer, a1LoopCount; // auto-advance on inactivity
  let a1DescPath, a1DescPI; //  road-following
  let a1PauseT; // silent pauses between narrative banners
  let a1OutroHoldT; // keeps final banner alive through blow-away
  let a1LastBannerText, a1LastBannerColor; // last narrative line, for outro re-hold
  let a1WalkArmed; 
  let a1WalkPromptT; 
  let a1HasAdvancedDialogue; 
  let a1NoControlTries; 
  ...
  const A1_NAR_PACE = 1.4; // syncs nar/outro speed with banner durations
  const _a1Paced = (d) => (d || 3000) / A1_NAR_PACE;
  const _a1PacedSeq = (seq) => seq.map((l) => ({ ...l, d: _a1Paced(l.d) }));
  ...
  function initAct2() {
    ...
    a1FullPath = [...a1Path, ...a1PA[0], ...a1PA[1], ...a1PA[2]]; // full route, for reverse retrace
    a1PX = a1Path[0].x;
    a1PY = a1Path[0].y;
    a1PI = 0;
    ...
    a1PauseT = 0;
    a1OutroHoldT = 0;
    a1LastBannerText = null;
    a1LastBannerColor = null;
    a1WalkArmed = false;
    a1WalkPromptT = 0;
    ...
    const TARGET_NPC_COUNT = 28; // denser ambient world
    const MIN_SPACING = 6;
    ...
    for (const en of a1NPCs) {
      /* Encounter NPCs sit on horizontal roads */
  ...
  function _a1ResolveEndingChoice(idx, delayGaveUp, delayKeepLiving) {
    if (idx === 0) {
      ...
      setTimeout(() => {
        ...
        a1PauseT = 1200;
        setTimeout(() => {
          Banner.show(NQ[0].t, NQ[0].c, _a1Paced(NQ[0].d));
          a1NP = 1;
        }, 1200);
      }, delayGaveUp);
    } else {
      ...
      setTimeout(() => {
        ...
        a1St = "loop";
        // skips the raw spawn coordinate
        a1Path = [{ x: Math.round(a1PX), y: Math.round(a1PY) }, ...[...a1FullPath].reverse().slice(0, -1)];
        a1PI = 0;
        Footsteps.start();
      }, delayKeepLiving);
    }
  ...
  function updateAct2(dt) {
    ...
    if (a1St === "pause") {
      ...
      // waits for banners before walking
    ...
    if (a1St === "walk") {
      if (a1PI >= a1Path.length - 1) {
        ...
        if (a1EI < a1NPCs.length) {
          ...
          Banner.show(NQ[a1NP].t, NQ[a1NP].c, _a1Paced(NQ[a1NP].d));
          a1NP++;
          a1ST2 = 0;
          a1St = "nar";
        } else {
      ...
      if (a1PI < a1Path.length - 1) {
        ...
        if (d < 0.3) {
          ...
          const s = 0.005 * A1_NAR_PACE * dt;
          _a1Move(a1PX + (dx / d) * s, a1PY + (dy / d) * s);
        }
      }
      // track pause delay separately
      if (a1PauseT > 0) {
        a1PauseT -= dt;
        return;
      }
      ...
      if (Banner.timer <= 0) {
        if (a1NP < NQ.length) {
          const q = NQ[a1NP];
          if (q.pause) {
            a1PauseT = _a1Paced(q.d);
          } else if (q.seq) {
            Banner.showSequence(_a1PacedSeq(q.seq), true);
          } else {
            Banner.show(q.t, q.c, _a1Paced(q.d), true);
          }
          a1NP++;
        } else {
          a1St = "done";
      ...
      if (a1ST2 > 800 && clickPending) {
        clickPending = false;
        /* Only accept clicks in choice box */
        ...
        /* click outside ignored; box stays visible */
      }
      if (a1ST2 > 800) {
      ...
      if (!Banner.seq && Banner.timer <= 0) {
        if (a1NP < NQ.length) {
          ...
          if (q.pause) {
            if (a1NP >= NQ.length) {
              // keeps banner visible through the blow-away
              Banner.show(a1LastBannerText, a1LastBannerColor, _a1Paced(q.d) + 500, true);
              a1OutroHoldT = _a1Paced(q.d);
            } else {
              a1PauseT = _a1Paced(q.d);
            }
          } else if (q.seq) {
            Banner.showSequence(_a1PacedSeq(q.seq), true);
            const _lastLine = [...q.seq].reverse().find((l) => !l.pause);
            if (_lastLine) {
              a1LastBannerText = _lastLine.t;
              a1LastBannerColor = _lastLine.c;
            }
            _a1SpawnDecayWave(0.5); // last banner beat
          } else {
            Banner.show(q.t, q.c, _a1Paced(q.d), true);
            a1LastBannerText = q.t;
            a1LastBannerColor = q.c;
      ...
      if (a1PI >= a1Path.length - 1) {
        // back at start — retrigger encounter 0
        Footsteps.stop();
        clickPending = false;
        audio.play("bump");
        const _encOX = Math.floor(a1CX),
        ...
        const tg = a1Path[a1PI + 1],
          ...
          dy = tg.y - a1PY,
          d = Math.sqrt(dx * dx + dy * dy);
        // impatience escalates with each loop
        const s = 0.02 * (a1LoopCount - 1) * dt;
        if (d < 0.3 || d <= s) {
          a1PX = tg.x;
          a1PY = tg.y;
          a1PI++;
  ...
  function _a1SpawnDecayWave(fraction, minCount = 6) {
    ...
    for (let y = 0; y < H; y++) {
      for (let x = 0; x < W; x++) {
        ...
        if (a1DecayClaimed.has(y * W + x)) continue;
  ...
  function _transitionAct2ToAct3() {
    ...
    const staged = _stageKeepCells([{ x: playerXY[0].x, y: playerXY[0].y, ch: "@", co: C_PLAYER }]);

    runActBoundary({
      outro: (done) => streamOut(() => setTimeout(done, 2000), { edge: "left", excludeXY: playerXY }), // beat of darkness before the next act
      setupNext: initAct3,
      banner: {
        lines: [
        ...
        frameIdx: 7, // rooflines frame
        keepCells: staged,
      },

      // same swap-in-from-the-right used for Act3→Act4
      intro: (done) => {
