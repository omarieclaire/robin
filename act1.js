  ...
  function startGame() {
    ...
    if (hasPlayed) {
      ...
      riseFromPile(
        ...
        { spawnPool: _titlePileSpawnPool, silent: true },
      );
      _titlePileSpawnPool = null; // one-time reuse; restarts use random heap
    }
    // hasPlayed becomes true at Act 3
  }
  ...
  // Hints Facebook in-app browser users to open natively
  ...
  startBtn.addEventListener("click", async () => {
    ...
    _startInFlight = true;
    window._markActivity(); // resets idle clock on play
    ...
    audio.preload(["trumpet", "paper", "playertxtbox", "npctxtbox"], { blob: true }).then(() => {
      audio.preload(["bump", "level", "drop", "urgent", "exit", "recruit", "narc", "grab", "death"], { blob: true });
    });

    setTimeout(() => audio.play("titleFall"), 700);
    overlay.classList.add("hidden");
  ...
  startBtn.addEventListener("mouseenter", () => {
    ...
    _playBtnHover = true; // landing anim loop reads this each frame
  });
  ...
  window.addEventListener("keydown", (e) => {
    ...
    if (overlay.classList.contains("hidden")) return; // in-game, Space is the action key
    if (helpOverlay.classList.contains("open")) return;
    if (startBtn.style.display === "none") return;
    if (document.activeElement === startBtn) return; // avoids double-firing the button's click
    if (phase && phase !== "done") return; // only landing or quit screen allowed
    e.preventDefault();
    startBtn.click();
  });
  ...
  const _showControlsHint = /[?&]ludo/.test(location.search);

  let _landingLayout = null;
  let _landingAmb = null; // ambient walkers and cats state
  ...
  function _layoutLanding() {
    ...
    if (!Device.isMobile) {
      ...
      const minRoof = topBandGround !== null ? topBandGround + 4 : 2; // clears top band's sidewalk and walkway
      const sideDefs = [
        { x0: 2, x1: frameX - 5 },
      ...
      for (const s of sideDefs) {
        ...
        if (frameBot - h1 + 1 < minRoof) continue; // skip if gutter too short
        ...
        const free = frameBot - minRoof + 1 - (h1 + h2 + 1); // gutter minus both rows and sidewalk
        if (free < 2) continue;
    ...
    _landingLayout = {
      ...
      frameCol: dullColor(PAL.landingFrame || "#f5a032", 0.25),
      btnCol: PAL.landingBtn || "#53ffaf", 
  ...
  function _spawnLandingWalker(midScreen) {
    ...
    A.walkers.push({
      ...
      dir,
      spd: (isCat ? 4.5 : 3) + Math.random() * 2, 
      art: isCat ? [Util.pick(CAT_GLYPHS)] : Util.pick(window.GAME_DATA.npcArts),
      co: isCat ? Util.pick(["#ee8833", "#c8b090", "#a8703a", "#e8c44a"]) : Util.pick(window.GAME_DATA.npcColors), // matches Act 3 cat's warm palette
      isCat,
    });
    A.nextWalkerIn = 4000 + Math.random() * 6000;
  }

  const CTRL_MARQUEE_SPEED = 5;
  function _drawControlsMarquee(nowMs) {
    const raw = window.LANG.landingControlsHint || "";
    if (!raw) return;
    const unit = raw + "   ★   ";
    const period = unit.length;
    const offset = Math.floor((nowMs / 1000) * CTRL_MARQUEE_SPEED) % period;
    const repeats = Math.ceil(W / period) + 2;
    grid.text(unit.repeat(repeats), -offset, H - 1, C_GOLD);
  }
  ...
  function _drawLandingFrame(nowMs) {
    ...
    if (_showControlsHint) _drawControlsMarquee(nowMs);
