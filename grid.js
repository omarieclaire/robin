...
(function () {
  /* Chars that render wider than one cell. */
  function isWideChar(ch) {
    if (!ch || ch.length === 0) return false;
    const code = ch.charCodeAt(0);
    /* Fast path for common ASCII/Latin */
    if (code < 0x0180) return false;
    /* Ranges widened for Windows Chrome fonts */
    return (
  ...
  class Grid {
    ...
    set(x, y, ch, co, bold, flip, lift, rot, scale) {
      ...
      if (x >= 0 && x < this.w && y >= 0 && y < this.h) {
        ...
        /* Rotated/scaled cells need own <span> too */
    ...
    htmlRows() {
      ...
      for (let y = 0; y < this.h; y++) {
        if (this.wideRows.has(y)) {
          /* Per-cell rendering for wide-char rows */
          for (let x = 0; x < this.w; x++) {
            const c = this.c[y][x];
          ...
          /* Fast path: merges spans by run-length */
          let lc = null,
            lb = false,
            lbg = null,
            llift = false,
