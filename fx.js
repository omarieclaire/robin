...
(function () {
  ...
  const Effects = (() => {
    const active = [];
    const HARD_CHARS = "█▓▒░▄▀■◆●✕#@!?%$&*XZ╬╫┼±";
    const BORDER_GLITCH_CHARS = "#%$&╬╫┼±/\\|=~^";
    const GLITCH_COLS = ["#f44", "#0ff", "#ff0", "#f0f", "#fff", "#f80", "#cc6688"];
    const RED_GLITCH_COLS = ["#ff5555", "#ff2222", "#cc3333", "#ff7777"];
    ...
    class DrainFX {
      ...
      update(dt, grid) {
        ...
        // first half drains, second restores
        const isRestoring = this.opts.restore && progress > 0.5;
        if (isRestoring) {
          const restoreT = (progress - 0.5) * 2;
          for (let y = 0; y < grid.h; y++) {
            for (let x = 0; x < grid.w; x++) {
          ...
          // evaporates from floor unless restoring
        ...
        if (progress >= 1) {
          // snap stays drained; caller redraws later
          this.done = true;
        }
      }
    }
    ...
    class MirrorFX {
      ...
      update(dt, grid) {
        this.t += dt;
        const peakAt = this.opts.peakAt ?? 0.5;
        const holdMs = this.opts.holdMs ?? 0;
        const rampMs = this.opts.duration * peakAt;
        const fadeMs = this.opts.duration * (1 - peakAt);
        const totalMs = rampMs + holdMs + fadeMs;
        const progress = Math.min(this.t / totalMs, 1);

        const foldPhase = this.opts.toBlack
          ? Math.min(1, progress / 0.5)
          : this.t < rampMs
          ? this.t / rampMs
          : this.t < rampMs + holdMs
          ? 1
          : 1 - (this.t - rampMs - holdMs) / fadeMs;
        const r = this.opts.radius;
        ...
        // row-slice avoids full-grid copies
        ...
        const smear = this.opts.toBlack
          ? Math.ceil(foldPhase * regionH * 0.5)
          : Math.ceil(foldPhase * regionH * 0.65 * (r == null ? 1 : this.opts.intensity));
          ...
          for (let x = 0; x < grid.w; x++) {
            ...
            if (fold < smear) {
              ...
              if (Math.random() < corruptChance) {
                const cols = this.opts.cols || GLITCH_COLS;
                grid.set(
                  x,
                  y,
    ...
    class CorruptFX {
      ...
      update(dt, grid) {
        ...
        if (this.opts.borderW) {
          const bw = this.opts.borderW;
          const bh = this.opts.borderH ?? bw;
          const redDelayMs = this.opts.redDelayMs ?? 0;
          const redRampMs = this.opts.redRampMs ?? 0;
          const redBias = redRampMs > 0 ? Math.max(0, Math.min(1, (this.t - redDelayMs) / redRampMs)) : 0;
          if (!this.stuck) {
            this.stuck = [];
            for (let y = 0; y < grid.h; y++) this.stuck[y] = new Array(grid.w).fill(null);
          }
          for (let y = 0; y < grid.h; y++) {
            for (let x = 0; x < grid.w; x++) {
              const inBorder = !(x >= bw && x < grid.w - bw && y >= bh && y < grid.h - bh);
              if (!inBorder && grid.c[y][x].ch !== " ") continue; // never glitch over the message box
              const cell = this.stuck[y][x];
              if (cell && cell.ms > 0) {
                // fixed char/color per hold, not re-rolled
                cell.ms -= dt;
                grid.set(x, y, cell.ch, cell.co);
                continue;
              }
              const startChance = inBorder ? this.opts.intensity * 0.06 : this.opts.intensity * 0.0012;
              if (Math.random() < startChance) {
                const goesDark = Math.random() < 0.25;
                const cols = Math.random() < redBias ? RED_GLITCH_COLS : this.opts.cols || GLITCH_COLS;
                const chars = BORDER_GLITCH_CHARS;
                this.stuck[y][x] = {
                  ms: inBorder ? 250 + Math.random() * 500 : 120 + Math.random() * 200,
                  ch: goesDark ? " " : chars[Math.floor(Math.random() * chars.length)],
                  co: goesDark ? "#000" : cols[Math.floor(Math.random() * cols.length)],
                };
              }
            }
          }
        ...
        // row swaps; disable via swap:false
        ...
        const bleedCols = this.opts.cols || GLITCH_COLS;
        for (let y = minY; y <= maxY; y++) {
          for (let x = 0; x < grid.w; x++) {
            if (!inRegion(x, y, this.opts)) continue;
            const f = falloff(x, y, this.opts);
            if (Math.random() < chaos * f * 0.45) {
              grid.set(x, y, HARD_CHARS[Math.floor(Math.random() * HARD_CHARS.length)], bleedCols[Math.floor(Math.random() * bleedCols.length)]);
            }
          }
        }
        if (this.opts.toBlack) {
    ...
    const REGISTRY = {
      ...
      mirror: MirrorFX,
      corrupt: CorruptFX,
      ringSnapFinal: RingSnapCascadeFX, // final narc takedown, full-scene cascade
      ringSnapLocal: RingSnapCascadeFX, // regular hit, localized snappy cascade
      colorPulseHit: PendingFX, // regular hit: localized red color pulse
      scalePulseGrocery: PendingFX, // grocery store: slow breathing scale ramp
      densityRampFinal: PendingFX, // final 10s before death: density ramp
      glitchOrbChase: PendingFX, // chase scene: glitch orb follows player
      typewriterReveal: PendingFX, // Act 2 opening: world decodes in
      magnetLean: MagnetLeanFX, // anticipation beat before streamOut() handoff
    };
    ...
    return {
      start(name, opts) {
        ...
        const fx = new Ctor({
          ...
          peakAt: opts.peakAt ?? 0.5,
          holdMs: opts.holdMs ?? 0,
          borderW: opts.borderW ?? 0,
          borderH: opts.borderH ?? 0,
          redDelayMs: opts.redDelayMs ?? 0,
          redRampMs: opts.redRampMs ?? 0,
          ...
          cols: opts.cols || null, // override palette, e.g. death screens
