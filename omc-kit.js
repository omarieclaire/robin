...
class Audio_ {
  constructor({ basePath = 'sounds/', poolSize = 8, volume = 1.0 } = {}) {
    ...
    this._fades = new Map();
    this._gainNodes = new WeakMap(); // audio element → GainNode (iOS Safari ignores .volume, so real volume goes through here)
  ...
  _loadOne(name, file, asBlob) {
    ...
    const wire = (audio, resolve) => {
      audio.oncanplaythrough = () => {
        audio._omcCached = true; // cache originals must never enter the pool — pool reuse rewrites src
  ...
  // Change the volume of an already-playing sound (e.g. a live music slider).
  ...
  play(name, { volume, loop = false, rate } = {}) {
    ...
    if (gainEntry) {
      ...
      audio.volume = 1; // gain node owns real output now — leave this at max everywhere it does still apply
    } else {
